...
async def send_sms(notification: SMSNotification):
    """SNS로 SMS 발송"""
    try:
        response = sns_client.publish(PhoneNumber=notification.phone_number, Message=notification.message)
...
async def notify_booking_confirmed(
    booking_id: str, user_email: EmailStr, event_title: str, background_tasks: BackgroundTasks
):
    """예약 확정 알림"""
