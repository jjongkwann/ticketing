...
from app.schemas import BookingCreate, BookingConfirm, BookingResponse, BookingListResponse
...
async def create_booking(
    ...
    try:
        ...
        if not reserve_result.get('success'):
            raise HTTPException(
                ...
                detail=reserve_result.get('message', 'Failed to reserve seat')
            )
    ...
    try:
        ...
        await inventory_client.release_seat(
...
async def confirm_booking(
    ...
    try:
        updated_booking = await dynamodb_repo.update_booking_status(
            booking_id,
            'confirmed',
            confirm_data.payment_id
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
